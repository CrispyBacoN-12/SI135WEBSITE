...
function hasKeyword(summary: string) {
  ...
  // ถ้ามีคำว่า BLOCK ให้ข้ามเลย
  if (text.includes("BLOCK")) return false;

  // ถ้า match keyword อื่น ๆ
...
export default function Examination() {
  ...
  useEffect(() => {
    async function fetchEvents() {
      ...
      const url = `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events?key=${apiKey}&timeMin=${timeMin}&maxResults=100&orderBy=startTime&singleEvents=true`;
          ...
          {events.map((e, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                <h4 className="font-bold justify-start basis-1/2 break-words whitespace-normal">{e.summary}</h4>
