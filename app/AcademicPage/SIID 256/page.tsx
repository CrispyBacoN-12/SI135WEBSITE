...
const SIID255 = () => {
  ...
  const sectionNames = [
    "Cranial nerve & Organs of special senses",
    "Circulatory system",
    "Respiratory system",
    "Gastrointestinal & Hepatobiliary system",
    "KUB System, Fluid & Electrolyte Balance",
    "Reproductive System",
  ];
  ...
  const [summativeList3, setSummativeList3] = useState([]);
  ...
  const [isSum3Open, setSum3IsOpen] = useState(false);
  ...
  return (
    <>
          ...
          <div className="text-left space-y-1 font-preuksa">
            ...
            <div className="flex gap-2 items-center flex-wrap">
              ...
              >
               ...
               <a
                href="https://www.youtube.com/playlist?list=PLnWAv0tkYmKEIpfnUd8Z5pMAnrkGXbQ9F"
                target="_blank"
                rel="noopener noreferrer"
                className="border border-slate-400 text-lg rounded-lg py-1 px-2 w-fit bg-gradient-to-r from-gray-200 to-gray-300 bg-transparent hover:bg-slate-200 transition-colors flex items-center gap-1"
              >
      ...
      <div className=" mx-auto">
        ...
        <div className="bg-gradient-to-r from-green-100 to-blue-100 shadow-md mt-8 overflow-hidden">
          <button
            onClick={() => setSum3IsOpen(!isSum3Open)}
            className="w-full text-left px-4 py-6 text-3xl font-bold text-sky-900 focus:outline-none"
          > Summative Examination 3</button>

        </div>

        {isSum3Open && (
          <div className="flex flex-col gap-4 mt-4">
            {summativeList3.map((lec, idx) => <SummativeCard key={idx} {...lec} />)}
          </div>
        )}
      </div>
    </>
